...
def add_new_asset_slides(prs: Presentation) -> None:
    assets_dir = ROOT / "docs" / "assets"

    # Slide Venn Diagram (After slide 3 -> Index 3)
    venn_path = assets_dir / "Context_engineering_Venn_Diagram.jpeg"
    if venn_path.exists():
        layout = min(prs.slide_layouts, key=lambda item: len(item.placeholders))
        slide_venn = prs.slides.add_slide(layout)
        for placeholder in list(slide_venn.placeholders):
            remove_shape(placeholder)
        background = slide_venn.background.fill
        background.solid()
        background.fore_color.rgb = RGBColor.from_string(PALETTE["white"])

        kicker = slide_venn.shapes.add_textbox(Inches(0.75), Inches(0.40), Inches(6.5), Inches(0.24))
        set_text(kicker, "04 / ARQUITECTURA DE CONTEXT ENGINEERING", "Aptos", 12, PALETTE["violet"], True)

        title = slide_venn.shapes.add_textbox(Inches(0.75), Inches(0.76), Inches(11.85), Inches(0.64))
        set_text(title, "El Ecosistema Completo de Context Engineering", "Aptos Display", 34, PALETTE["ink"], True)

        divider = slide_venn.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.RECTANGLE,
                                         Inches(0.75), Inches(1.55), Inches(11.83), Inches(0.012))
        divider.fill.solid()
        divider.fill.fore_color.rgb = RGBColor.from_string(PALETTE["line_light"])
        divider.line.fill.background()

        add_picture_contain(slide_venn, venn_path, Inches(0.75), Inches(1.70), Inches(11.83), Inches(4.50), align="center", valign="center")

        exp_box_venn = slide_venn.shapes.add_textbox(Inches(0.75), Inches(6.30), Inches(11.83), Inches(0.65))
        set_text(exp_box_venn,
                 ...
                 font_name="Aptos", size=14, color=PALETTE["body_light"], bold=False, alignment=PP_ALIGN.CENTER)
        
        # Mueve la diapositiva a la posición 3
        slide_id_list = prs.slides._sldIdLst
        new_id = slide_id_list[-1]
        slide_id_list.remove(new_id)
        slide_id_list.insert(3, new_id)

    # Slide RAG Formal (After slide 9 (now 10 since we added Venn) -> Index 10)
    rag_path = assets_dir / "RAG Formal.png"
    if rag_path.exists():
        layout = min(prs.slide_layouts, key=lambda item: len(item.placeholders))
        slide_rag = prs.slides.add_slide(layout)
        for placeholder in list(slide_rag.placeholders):
            remove_shape(placeholder)
        background = slide_rag.background.fill
        background.solid()
        background.fore_color.rgb = RGBColor.from_string(PALETTE["white"])

        kicker = slide_rag.shapes.add_textbox(Inches(0.75), Inches(0.40), Inches(6.5), Inches(0.24))
        set_text(kicker, "11 / FORMALIZACIÓN PROBABILÍSTICA DE RAG", "Aptos", 12, PALETTE["violet"], True)

        title = slide_rag.shapes.add_textbox(Inches(0.75), Inches(0.76), Inches(11.85), Inches(0.64))
        set_text(title, "Modelo Probabilístico DAG de Recuperación-Generación", "Aptos Display", 34, PALETTE["ink"], True)

        divider = slide_rag.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.RECTANGLE,
                                         Inches(0.75), Inches(1.55), Inches(11.83), Inches(0.012))
        divider.fill.solid()
        divider.fill.fore_color.rgb = RGBColor.from_string(PALETTE["line_light"])
        divider.line.fill.background()

        add_picture_contain(slide_rag, rag_path, Inches(0.75), Inches(1.70), Inches(11.83), Inches(4.50), align="center", valign="center")

        exp_box_rag = slide_rag.shapes.add_textbox(Inches(0.75), Inches(6.30), Inches(11.83), Inches(0.65))
        set_text(exp_box_rag,
        ...
        # Mueve la diapositiva a la posición 10
        slide_id_list = prs.slides._sldIdLst
        new_id = slide_id_list[-1]
        slide_id_list.remove(new_id)
        slide_id_list.insert(10, new_id)
...
def validate(prs: Presentation) -> None:
    if len(prs.slides) != 25:
        raise ValueError(f"Se esperaban 25 diapositivas y se encontraron {len(prs.slides)}")
